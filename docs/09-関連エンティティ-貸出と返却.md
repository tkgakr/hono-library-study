...
保存側（`save`）は book と同じ「CREATE は insert、それ以外は update」の形ですが、**返却の update には `returnedOn IS NULL` を付けます**。

```ts
// src/api/infrastructure/database/repository/loanRepository.ts （save の返却部分）
