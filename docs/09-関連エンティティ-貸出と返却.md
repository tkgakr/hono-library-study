...
export const loanCreateUsecase = async (
  ...
  return ok(createLoan(validatedEntity))
    .asyncAndThen((created) => ResultAsync.fromPromise(repository.save(created), promiseErrorReturn(logger, ResultCodes.E0303)))
    .andThen(falsyValueCheck(ResultCodes.E0303))
}
```
